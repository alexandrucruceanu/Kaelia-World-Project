...
**Project Context:**
This is a realistic fictional world recovered from a 2014 archive. It combines 3D modelled geography (Cinema4D/Fractal Terrains) with simulated climate data. We are NO LONGER building a solar system; the focus is exclusively on this single planet. **Version 3.8.0** maintains the clean 3-tier data hierarchy (**Continent → Country → City**) with 5 continents, 36 countries, and 61 cities. Recent updates include **Gemini Model Upgrade** (`gemini-3-flash-preview` for JSON, `gemini-3-pro-image-preview` for images), **Navigation Integration** (Map ↔ Prompt Explorer deep-linking), **Add City via Prompt Explorer**, and **UI Cleanup**.
...
5.  **Tooling:** `check_assets.js` validates all city image references. `generate_assets_hybrid.py` handles batch and on-demand AI image generation (landscapes via `gemini-3-pro-image-preview`, heraldry and JSON via `gemini-3-flash-preview`).
